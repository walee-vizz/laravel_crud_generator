        ...
        <div class="col">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('activity_log.update', $activity_log) }}" method="POST">
                        @csrf
                        ...
                        <select name="user_id" id="user_id" @error('user_id') is-invalid @enderror
                            ...
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}" {{ $user->id === $activity_log->user_id ? 'selected' : '' }}>{{ $user->name }}</option>
                            @endforeach
                        </select>
                        @error('user_id')
                        ...
                        <input type="text" class="form-control @error('activity_type') is-invalid @enderror "
                            name="activity_type" id="activity_type" value="{{ old('activity_type') ? old('activity_type') : $activity_log->activity_type}}">
                        @error('activity_type')
                            <div class="alert alert-danger"> {{ $message }} </div>
                        @enderror
                        <label for="description">Description</label>
                        <textarea type="text" class="form-control @error('description') is-invalid @enderror " name="description"
                            id="description">{{ old('description') ? old('description') : $activity_log->description }}</textarea>
                        @error('description')
                            <div class="alert alert-danger"> {{ $message }} </div>
                        @enderror
